...
* Пусть $$S \subseteq \mathbb{R}^n$$ - аффинное множество. Пусть так же имеются точки $$\mathbf{y} \in \mathbb{R}^n$$ и $$\mathbf{\pi} \in S$$. Тогда $$\pi$$ является проекцией точки $$\mathbf{y}$$ на $$S$$, т.е. $$\pi_S (\mathbf{y}) = \pi$$ тогда и только тогда, когда для всех $$\mathbf{x} \in S$$ справедливо равенство: 
