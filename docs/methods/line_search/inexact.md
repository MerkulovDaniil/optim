...
$$
\phi_1(\alpha) = f(x_k) - \beta_1 \alpha \|\nabla f(x_k)\|^2
$$
and
$$
\phi_2(\alpha) = f(x_k) - \beta_2 \alpha \|\nabla f(x_k)\|^2
$$

Note, that Goldstein-Armijo conditions determine the location of the function $$\phi(\alpha)$$ between $$\phi_1(\alpha)$$ and $$\phi_2(\alpha)$$. Typically, we choose $$\beta_1 = \rho$$ and $$\beta_2 = 1 - \rho$$, while $$ \rho \in (0.5, 1)$$.
