...
def compute_cg_trajectory(A, x0, iterations=7):
    """
    Compute the Conjugate Gradient trajectory for the quadratic function
    f(x) = 0.5 * x^T A x starting from x0.
    """
    trajectory = [x0.copy()]
    x = x0.copy()
    r = -A @ x  # negative gradient is the residual (since b=0)
    p = r.copy()
...
# Plot CG trajectory for κ=1
traj0_cg = compute_cg_trajectory(A0, x0, iterations=7)
ax1.plot(traj0_cg[:, 0], traj0_cg[:, 1], 'g-o', lw=1.5, markersize=3, label='CG', zorder=10)
...
# Plot CG trajectory for κ=100
traj_max_cg = compute_cg_trajectory(A_max, x0, iterations=7)
ax2.plot(traj_max_cg[:, 0], traj_max_cg[:, 1], 'g-o', lw=1.5, markersize=3, label='CG', zorder=10)
...
# Initial CG trajectory plot
traj0_cg = compute_cg_trajectory(A0, x0, iterations=7)
line_traj_cg, = ax.plot(traj0_cg[:, 0], traj0_cg[:, 1], 'g-o', lw=1.5, markersize=3, 
                        label='CG', zorder=10)
...
ax.legend(fontsize=8, bbox_to_anchor=(0.8 , -0.1), frameon=False, ncol=3)
...
def update(frame):
    ...
    # Update CG trajectory
    traj_cg = compute_cg_trajectory(A, x0, iterations=7)
    line_traj_cg.set_data(traj_cg[:, 0], traj_cg[:, 1])
    
    # Update title with fixed width format
    title.set_text(f"$\\varkappa={kappa:5.1f}$")
    
    return line_traj_gd, line_traj_accel, line_traj_cg, title
